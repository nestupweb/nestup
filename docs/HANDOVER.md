...
- **Supabase project** `eiykciushbnbwpxpvybi` (keys in `.env.local`, gitignored). RLS on every table. Migrations `supabase/migrations/0001…0015` — **all 15 are applied to the live project** (latest: `0015_message_client_id_next_viewing`). The Supabase MCP tools (`execute_sql`, `apply_migration`) work from Claude Code; always add the matching `.sql` file too.
- **Auth:** email + password, **email confirmation ON by my explicit decision — keep it**. Demo accounts skip confirmation.
...
- `npm test` — **use this, not `npx vitest`** (it sets `NODE_OPTIONS=--no-experimental-webstorage`; without it `theme-toggle.test.tsx` fails on Node 25). Currently **31 files / 158 tests passing**. `npx tsc --noEmit` and `npx eslint <paths>` are clean.
...
3. **Ops:** set Supabase Auth Site URL + redirect URLs to the Vercel domain (confirmation emails may still point at localhost); enable leaked-password protection; optionally set `GOOGLE_CLIENT_ID/SECRET` + `NEXT_PUBLIC_SITE_URL` on Vercel (redirect URI `https://nestup-kappa.vercel.app/api/google/callback`).
