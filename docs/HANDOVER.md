...
- `npm test` — **use this, not `npx vitest`** (it sets `NODE_OPTIONS=--no-experimental-webstorage`; without it `theme-toggle.test.tsx` fails on Node 25). Currently **33 files / 172 tests passing**. `npx tsc --noEmit` and `npx eslint <paths>` are clean.
