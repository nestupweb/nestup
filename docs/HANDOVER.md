...
- `npm test` — **use this, not `npx vitest`** (it sets `NODE_OPTIONS=--no-experimental-webstorage`; without it `theme-toggle.test.tsx` fails on Node 25). Currently **35 files / 187 tests passing**. `npx tsc --noEmit` and `npx eslint <paths>` are clean.
...
- Bottom floating nav (Swipe · Listings · Chat · Profile); header logo is my `Logo-NU.jpeg` rendered as a theme-coloured CSS mask (`components/ui/Logo.tsx`, `public/brand/nestup-wordmark.png`). Web-app manifest + Apple meta (`app/manifest.ts`), icons in `public/icons/`.
...
3. **Ops:** set Supabase Auth Site URL + redirect URLs to the Vercel domain (confirmation emails may still point at localhost) — the password-reset email needs `https://nestup-kappa.vercel.app/auth/confirm` (or `https://nestup-kappa.vercel.app/**`) in **Authentication → URL Configuration → Redirect URLs**, otherwise Supabase sends the link to the Site URL instead; enable leaked-password protection; optionally set `GOOGLE_CLIENT_ID/SECRET` + `NEXT_PUBLIC_SITE_URL` on Vercel (redirect URI `https://nestup-kappa.vercel.app/api/google/callback`).
