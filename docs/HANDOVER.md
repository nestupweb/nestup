...
- `npm test` — **use this, not `npx vitest`** (it sets `NODE_OPTIONS=--no-experimental-webstorage`; without it `theme-toggle.test.tsx` fails on Node 25). Currently **43 files / 224 tests passing**. `npx tsc --noEmit` and `npx eslint <paths>` are clean.
...
3. **Ops (still open, 2026-08-26 evening):** the Supabase Auth **Site URL is `http://localhost:3000`** and the allow-list has no Vercel entry (verified by probing `/auth/v1/verify`), so every emailed link still bounces to localhost. Fix = `npm run auth:config -- --apply` with a `SUPABASE_ACCESS_TOKEN` in `.env.local` (sets Site URL `https://nestup-kappa.vercel.app`, allow-list `…/**` + `http://localhost:3000/**`, the branded templates, and SMTP when `SMTP_*` are set) — or by hand in **Authentication → URL Configuration** + **Emails → Templates** (paste `supabase/templates/*.html`) + **Project Settings → Authentication → SMTP**. Without custom SMTP the built-in mailer only reaches team addresses at ~2 mails/hour. `NEXT_PUBLIC_SITE_URL` is set on Vercel production (2026-08-26). Also still open: leaked-password protection; `GOOGLE_CLIENT_ID/SECRET` (redirect URI `https://nestup-kappa.vercel.app/api/google/callback`).
