...
- `npm test` — **use this, not `npx vitest`** (it sets `NODE_OPTIONS=--no-experimental-webstorage`; without it `theme-toggle.test.tsx` fails on Node 25). Currently **44 files / 226 tests passing**. `npx tsc --noEmit` and `npx eslint <paths>` are clean.
