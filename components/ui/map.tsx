...
// mapcn ships this pointing at unpkg.com. Served from our own origin instead:
// `scripts/copy-maplibre-worker.mjs` (npm `prebuild`) keeps the two files in
// public/maplibre in step with the installed maplibre-gl, so drawing a map
// never depends on a third-party CDN being up.
...
function PopupCloseButton({ onClick }: { onClick: () => void }) {
  return (
    <button
      ...
      className="focus-visible:ring-ring hover:bg-hairline text-foreground absolute top-1 right-1 z-10 inline-flex size-5 cursor-pointer items-center justify-center rounded-sm transition-colors focus:outline-none focus-visible:ring-2 focus-visible:ring-inset"
    >
...
function MarkerPopup({
  ...
  return createPortal(
    <div
      className={cn(
        "bg-popover text-popover-foreground border-border relative max-w-62 rounded-md border p-3 shadow-md",
...
> = {
  ...
  pointColor?: string;
  /** Local addition (not upstream mapcn): radius of an unclustered point, in
   ...
   * which is unreadable on this app's dark-theme accent.
   */
...
>({
  ...
  useEffect(() => {
    ...
    // Local addition: the theme can change under a mounted map.
    if (prev.clusterTextColor !== clusterTextColor || prev.strokeColor !== strokeColor) {
      if (map.getLayer(clusterCountLayerId))
