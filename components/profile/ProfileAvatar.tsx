...
/** Extra profile pictures as small circles; each opens full-size, round. */
export function ProfileGallery({ photos, name }: { photos: string[]; name: string }) {
  ...
  return (
    <>
      <ul className="mt-4 flex flex-wrap gap-3" aria-label="More photos">
        {photos.map((url, i) => (
          <li key={url}>
            <button
              type="button"
