...
export function SwipeCard({
  ...
  return (
    <article
      aria-label={`${listing.title}, room ${position} of ${total}`}
    ...
    >
      ...
      >
        ...
        <span className="absolute right-4 top-7 rounded-full bg-black/40 px-2.5 py-1 text-[10px] font-semibold tracking-[0.16em] text-white/90 backdrop-blur-md">
          {position} / {total}
        </span>
        ...
        {/* Title + decisions along the bottom edge. */}
        <div className="absolute inset-x-0 bottom-0 flex items-end justify-between gap-4 p-5">
          <div className="min-w-0 text-white">
            ...
            <h1 className="mt-1 line-clamp-2 font-serif text-2xl font-semibold leading-tight sm:text-3xl">
              {listing.title}
            </h1>
          </div>
          <div className="flex shrink-0 items-center gap-3">
            <button
              ...
              className={`${stageButton} h-12 w-12 border border-white/70 bg-black/30 hover:bg-white hover:text-ink disabled:opacity-60 sm:h-14 sm:w-14`}
            >
            ...
            <button
              ...
              className={`${stageButton} h-12 w-12 bg-accent text-accent-contrast shadow-[0_12px_30px_-8px_rgba(0,0,0,0.6)] hover:scale-105 disabled:opacity-60 sm:h-14 sm:w-14`}
            >
