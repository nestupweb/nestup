...
/**
 * English, then any Latin spelling — and nothing at all if there is neither.
 *
 ...
 *
 * There is deliberately **no fall-back to the raw `name`**. That was the first
 * version, and it left the occasional Hebrew label on an otherwise English map
 * — a park in Acre, in the screenshot that caught it. The user asked for the
 * Hebrew gone, so a feature nobody has named in Latin script goes unlabelled
